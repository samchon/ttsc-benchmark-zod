...
test("tuple fills defaults for missing trailing elements", () => {
  ...
  expectTypeOf<typeof t._input>().toEqualTypeOf<
  ...
  // Prefault parity
  expect(
    z.tuple([z.string(), z.string().prefault("delta")]).parse(["alpha"]),
  ).toEqual(["alpha", "delta"]);

  // Defaults wrapped in modifiers: `optout` propagates through these, so the
  // fix is not type-name specific.
  expect(
    z.tuple([z.string(), z.string().default("x").nullable()]).parse(["alpha"]),
  ).toEqual(["alpha", "x"]);
  expect(
    z.tuple([z.string(), z.string().default("x").readonly()]).parse(["alpha"]),
  ).toEqual(["alpha", "x"]);
  expect(
    z.tuple([z.string(), z.string().default("x").catch("y")]).parse(["alpha"]),
  ).toEqual(["alpha", "x"]);
  expect(
    z
      .tuple([z.string(), z.string().default("x").pipe(z.string())])
      .parse(["alpha"]),
  ).toEqual(["alpha", "x"]);
});
...
test("tuple keeps length-1 array for missing `.optional()` elements", () => {
  ...
  expect(
    z.tuple([z.string(), z.undefined()]).safeParse(["alpha"]).error!.issues,
  ...
  `);
  expect(
    z.tuple([z.string(), z.undefined()]).parse(["alpha", undefined]),
  ).toHaveLength(2);

  // `.optional().nullable()` still trims — `.optional()` propagates the
  // optin/optout flags through the nullable wrapper.
  expect(
    z.tuple([z.string(), z.string().optional().nullable()]).parse(["alpha"]),
  ).toHaveLength(1);
...
test("tuple result is dense when optional precedes a default", () => {
  ...
  expect(
    z
      .tuple([z.string(), z.string().default("d"), z.string().optional()])
      .parse(["alpha"]),
  ).toEqual(["alpha", "d"]);
...
test("tuple rejects absent exact optional before defaulted output", () => {
  ...
  expectTypeOf<typeof schema._output>().toEqualTypeOf<
  ...
  expect(schema.parse(["alpha", "bravo"])).toEqual([
    "alpha",
    "bravo",
    "fallback",
  ]);
  ...
  expect(
    z
      .tuple([z.string(), z.string().exactOptional(), z.string().optional()])
      .parse(["alpha"]),
  ).toEqual(["alpha"]);
});
...
test("under-length tuple emits a single too_small with optStart minimum", () => {
  ...
  expect(interiorOptional.safeParse(["a", 1]).error!.issues)
    .toMatchInlineSnapshot(`
    [
      {
        "code": "too_small",
...
test("too_big tuple still surfaces element-wise type errors for present indices", () => {
  const schema = z.tuple([z.string(), z.number()]);
  expect(schema.safeParse([1, "x", "extra"]).error!.issues)
    .toMatchInlineSnapshot(`
    [
      {
        "code": "too_big",
